...
### Stage 6b: Create/Update Tag Notes (Semantic) ✅ ACTIVE

**STATUS**: MANDATORY - This stage MUST be executed for every conversation.

**CRITICAL**: You MUST use the Python scripts in scripts/tag_note_manager.py - DO NOT try to manually edit tag notes!

**IMPLEMENTATION**:

1. **Read the complete instructions**: `_system/stage-6b-tag-notes-prompt.md`
2. **Execute using Python**: Run the tag_note_manager.py script via Bash tool
3. **Do NOT manually create/edit tag notes** - the Python script handles everything

**For EACH entity extracted in Stage 1**, use tag_note_manager.py to create or update tag notes:
...
**Step 6b.6: CRITICAL - Verify Execution**

After completing this stage, verify:
1. All entities have corresponding tag notes
2. Folder hierarchy was created (Technology/, Language/, etc.)
3. Monthly entries were added with timestamps
4. Related tags wikilinks were included
5. Source conversation backlinks were added
6. Month rollover compression was checked

**DO NOT proceed to Stage 7 without completing all of Stage 6b.**

**HIERARCHY VERIFICATION (ADDED 2025-11-11):**
After creating each tag note, verify:
- Frontmatter includes `parent_tags:` field (list of parent tag identifiers)
- Content includes `## Hierarchy` section with parent wikilinks
- Example: `**Parent Categories**: [[Python]] > [[Web Frameworks]]`

This ensures graph view shows hierarchical parent-child relationships automatically.

See `_system/stage-6b-tag-notes-prompt.md` for detailed implementation with Python code examples.
...
**BEFORE creating completion signal**, create a detailed log file:
...
log_time = log_timestamp.strftime("%H%M%S")

# Get next log number
log_dir = "C:/Obsidian-memory-vault/docs/pipeline_agent"
existing_logs = [f for f in os.listdir(log_dir) if f.startswith("processing_log_")]
log_number = len(existing_logs) + 1
log_num_str = f"{log_number:04d}"

# Create log filename
log_filename = f"processing_log_{log_date}_{log_time}_{log_num_str}.md"
...
# Pipeline Processing Log #{log_number}

**Timestamp**: {log_timestamp.strftime("%Y-%m-%d %H:%M:%S")}

## Files Processed

{{LIST_ALL_FILES_PROCESSED_THIS_RUN}}
...
- **Total Tag Notes**: {{TOTAL_TAG_NOTES}}
- **Processing Duration**: {{DURATION_SECONDS}} seconds

## Stage Completion

- [x] Stage 1: Entity Extraction
- [x] Stage 2: Tag Assignment
- [x] Stage 3: Area Matching
- [x] Stage 4: Time Estimation
- [x] Stage 5: Novelty Detection
- [x] Stage 6a: Conversation Node Creation
- [x] Stage 6b: Tag Notes Update
- [x] Stage 7: Bidirectional Linking
- [x] Stage 8: Finalization

## Errors & Warnings

{{IF_ANY_ERRORS_LIST_THEM_HERE}}
...
{{IF_NEW_AREAS_DISCOVERED}}
- Review new areas in: _system/new-areas-queue.md
{{END_IF}}

{{IF_MONTHLY_CONSOLIDATION_NEEDED}}
- Monthly consolidation pending (last day of month)
{{END_IF}}

## File Details

{{FOR_EACH_FILE_PROCESSED}}
### {{FILE_NAME}}
- **Entities**: {{ENTITY_COUNT}}
- **Relationships**: {{RELATIONSHIP_COUNT}}
- **Tags**: {{TAG_LIST}}
- **Duration**: {{ACTIVE_TIME_MINUTES}} minutes
- **Novelty Score**: {{NOVELTY_SCORE}}
- **Primary Area**: {{PRIMARY_AREA_PATH}}
{{END_FOR}}

---
*Generated by Processing Pipeline Agent*
*Log stored in: docs/pipeline_agent/*
"""
...
print(f"✅ Processing log created: {log_filename}")
```

### Signal Completion and Stop Agent

**CRITICAL - COMPLETION SEQUENCE**:

After creating the processing log, agent MUST signal completion and exit:

**Step 1: Write completion signal**
```bash
...
**Purpose**:
- Processing log provides detailed audit trail
- Completion signal triggers file watcher notification
- File watcher removes signal after reading
- File watcher notifies user that processing is complete
- Agent exits cleanly to free resources
- Prevents agent from running indefinitely

**Failure to create log OR signal = FAILED finalization**
